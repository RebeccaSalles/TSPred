...
#' Automatic prediction with wavelet transform %% ~~function to do ... ~~
#' 
...
#' 2nd position, etc.) on each calculated ranking criteria. %% ~~ If necessary,
...
#' 'Details'. %% ~~Describe \code{filtered} here~~
...
#' @seealso \code{\link{fittestEMD}}, \code{\link{fittestMAS}} %% ~~objects to
...
fittestWavelet <- 
  function(timeseries, timeseries.test=NULL, h=1, filters=c("haar", "d4", "la8", "bl14", "c6"), n.levels=NULL, maxlevel=NULL, 
           boundary="periodic",model=c("ets","arima"), conf.level=0.95, na.action=na.omit,
