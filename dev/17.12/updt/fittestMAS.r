...
fittestMAS <- 
  function(timeseries, timeseries.test=NULL, h=NULL, order=NULL, minorder=1, maxorder=min(36,length(ts(na.action(timeseries)))/2), 
    ...
    optim.model <- function(timeseries,order,modelType,n.ahead,level,max.d=NULL,max.D=NULL,stationary=NULL){
    ...
    if(is.null(order)){
      ...
      for(par in pars.opt){
        ...
        fc <- optim.model(ts.tmp,order=par,modelType=modelType,n.ahead=n.ahead,level=level,max.d=max.d,max.D=max.D,stationary=stationary)
        model <- fc$model
    ...
    #generates and optimizes Model based on optim parameter values
      ...
      fc <- optim.model(ts,order=order.optim,modelType=modelType,n.ahead=n.ahead,level=level,max.d=max.d,max.D=max.D,stationary=stationary)
      model <- fc$model
    }
