...
fittestEMD <- 
  function(timeseries, timeseries.test=NULL, h=NULL, max.imf=10, boundary=NULL, level=0.95, na.action=na.omit,
    ...
    if(!is.null(boundary))
      if(!(boundary %in% c("none","wave","symmetric","periodic","evenodd"))) stop("the boundary argument must be NULL or a subset of c('none','wave','symmetric','periodic','evenodd')")
    ...
    residue.pred <- function(emdt,n.ahead,level,rank.by){
      fpoly <- fittestPolyR(emdt$residue, h=n.ahead, level=level, se.fit=TRUE, rank.by=rank.by)
      return(list(pred=fpoly$pred, model=fpoly$model))
    }
    ...
    pred.criteria <- function(varModel,n.ahead,level,predResidue,ts.test,ts){
      ...
      pred <- list(pred=pred$fcst,lower=pred$lower,upper=pred$upper)
    ...
    for(bndr in boundary){
      ...
      for(firstMeaningfulImf in firstMeaningfulImf.opt){
        ...
        if(any(c("AIC","AICc","BIC","logLik") %in% rank.by)){
          ...
          rank.measures <- pred.criteria(varModel,n.ahead,level,predResidue,ts.val,ts.tmp)$errors
        }
    ...
    #computes predictions, and prediction error measures (if timeseries.test is provided)
    pred.measures <- pred.criteria(varModel,n.ahead,level,predResidue$pred,ts.test,ts)
    ...
    results <- c( list(model=emdt), meaningfulImfs=str.meaningfulImfs, boundary=boundary.optim, varImfs=varModel, 
                  polyRresidue=predResidue$model, fit.measures, list(pred=prediction), errors.measures )
