fittestTransform <-
  function(timeseries, timeseries.test=NULL, h=NULL, na.action=na.omit, 
           trans=c("original","LT","LT10","BCT","PCT","MAS","DT","DIF","SDIF","DIFs","SM","ETS","HW","TF","WT","EMD"),
    ...
    if("original" %in% trans){
      Tr[["original"]] <- tryCatch( fittestArimaTrans(timeseries,timeseries.test,h, na.action=na.action, max.d=0, max.D=0, stationary=TRUE, 
