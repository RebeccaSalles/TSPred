fittestPolyR <- 
  function(timeseries, timeseries.test=NULL, h=NULL, order=NULL, minorder=0, maxorder=5, raw = FALSE, na.action=na.omit, level=0.95, se.fit=FALSE,
    ...
    pred.criteria <- function(model,data.ahead,level,se.fit,i.n.ahead,ts.test,ts){
      #computes predictions using the candidate model
      pred <- predict(model, data.ahead, se.fit=se.fit, level=level)
      if(se.fit) pred.mean <- ts(pred$fit,start=i.n.ahead)
      else {
        pred.mean <- ts(pred,start=i.n.ahead)
        pred <- pred.mean
      }
    ...
    for(i in 1:length(calls)){
      ...
      if(any(c("MSE","NMSE","MAPE","sMAPE","MaxError") %in% rank.by)){
        ...
        errors <- pred.criteria(model,data.val[-1],level,TRUE,nrow(data.tmp)+1,data.val[["y"]],data.tmp[["y"]])$errors
    ...
    #computes predictions, and prediction error measures (if timeseries.test is provided)
    pred.measures <- pred.criteria(model,data.test,level,se.fit,nobs+1,ts.test,ts)
