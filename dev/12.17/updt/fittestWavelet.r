...
fittestWavelet <- 
  function(timeseries, timeseries.test=NULL, h=NULL, filters=c("haar", "d4", "la8", "bl14", "c6"), n.levels=NULL, maxlevel=NULL, 
           model=c("ets","arima"), max.d=2, max.D=1, stationary=FALSE, conf.level=0.95, na.action=na.omit,
    ...
    optim.models <- function(timeseries,filter,maxlevel,modelType,max.d=NULL,max.D=NULL,stationary=NULL){
      #decompose the time series by maximal overlap discrete wavelet transform
      ...
      for(level in 1:maxlevel){
        if(modelType=="ets") {
          ...
          Wmodels[[level]] <- forecast::auto.arima(wdecomp@W[[level]], max.d=max.d, max.D=max.D, stationary=stationary)
          Vmodels[[level]] <- forecast::auto.arima(wdecomp@V[[level]], max.d=max.d, max.D=max.D, stationary=stationary)
        }
      }
    ...
    pred.criteria <- function(wdecomp,decomp.level,prediction,n.ahead,i.n.ahead,ts.test,ts){
      ...
      if(!is.null(ts.test)) {
        ...
        return(list(pred=list(pred=pred,lower=lower,upper=upper),errors=data.frame(MSE=MSE,NMSE=NMSE,MAPE=MAPE,sMAPE=sMAPE,MaxError=MaxError)))
      }
      
      return(list(pred=list(pred=pred,lower=lower,upper=upper)))
    }
    ...
    if(length(filters)==1 & !is.null(n.levels)){
      ...
      for(filter in filters){
        ...
        models <- optim.models(ts.tmp,filter,maxlevel,modelType,max.d=max.d,max.D=max.D,stationary=stationary)
    ...
    #decompose time series up to maxlevel and optimize Models of each decomposed series given a set of initial parameters
    models <- optim.models(ts,filter.optim,level.optim,modelType,max.d=max.d,max.D=max.D,stationary=stationary)
